...
public class Speaker extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        request.getRequestDispatcher("Speaker.jsp").forward(request, response);
	}
	...
	private String doCmdLogin(HttpServletRequest request, HttpServletResponse response) {
		...
		if (name == null || name.length() == 0) {
			...
			SpeakerContext speakerContext = new SpeakerContext(name, ipAddress);
			if (_serverContext.loginMember(speakerContext) == null) {
				output = String.format("##Err##: login failure for speaker '%s'!", speakerContext.toString());
			} else {
				output =  String.format("Speaker '%s' logged in!", speakerContext.toString()); 
			}
		}
	...
	private String doCmdLogout(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if (memberContext == null) {
			...
			output =  String.format("Speaker '%s' logged out!", memberContext.toString()); 
		}
